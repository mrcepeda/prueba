...
installpath=$(ps -ef | grep kernel  | grep "vt-net/" | tail -n1 | awk '{ print $10 }' |  awk -F"vt-net/" '{ print $1 }')
...
installpathweblogic=$(ps -ef | grep -i "weblogic" | grep -v grep | tail -n1 )
...
# CORE
vtconsoleCoreCF="$installpath/vt-console/config/application.properties"
vt724CoreCF="$installpath/vt-724/vt-config/config/version.info"
...
arr=( $(/usr/bin/sudo -i -u vt724 grep -v ^\# $installpath/share/network/admin/tnsnames.ora | egrep -v "DESCRIPTION|CONNECT_DATA|SERVER" | grep -A 4 $2)  )
...
OS=$(grep -i pretty /etc/*release | awk -F "\"" '{ print $2 }')
IP8=$(hostname -I)
echo $1\_SISTEMAOPERATIVO';' $OS

if [[ $OS == *Linux*8.* ]]; then
  echo "$1_IPADDRESS; $IP8"
  else
  echo "$1_IPADDRESS; $IP"
fi
...
VTGWF=$(cat /tmp/TeMp | grep gateway | grep conf | awk -F"(" '{ print $2 }' | cut -d":" -f1 | grep -v ^\# | uniq | xargs grep vt-gateway.php | cut -d":" -f1)
...
VTGWFiles=0
  ...
  for i in ${VTGWF[@]}
   do  VTGWFile=$(basename $i) ; ((VTGWFiles++)) ; TEST=$(/usr/bin/sudo -i -u $installusercomm grep vt-gateway.php $i | grep -v ^\# | grep "/vt-gateway/" | uniq | awk '{ print $3 }' | uniq )
   export VTGWFile
 
  if [ $? == "0" ] || [[ ! -z "$TEST" ]];
  then
       VTGWVER=$(/usr/bin/sudo -i -u $installusercomm wc -l $TEST | awk '{ print $1 }')

   if [ $VTGWVER = 392 ];
    then
      if [ ${VTGWF[@]} -ge 1 ]; then
        echo $1\_VT-GATEWAY_$VTGWFile\_VERSION';' 1.14.0
      else
        echo $1\_VT-GATEWAY_VERSION';' 1.14.0
      fi
   fi
   ...
   if [ $VTGWVER != 392 ] && [ $VTGWVER != 0 ];
    then
      if [ $VTGWFiles -ge 2 ]; then
        echo $1\_VT-GATEWAY_$VTGWFile\_VERSION';' $VTGWVER
      else
        echo $1\_VT-GATEWAY_VERSION';' $VTGWVER
      fi
   fi

   else
     echo $1\_VT-GATEWAY_$VTGWFile\_VERSION';'NA >&2
  fi
...
if [ -f "$httpdCOMM" ];
then
 echo COMM_VT-GATEWAY_SERVERNAME \; $(cat /tmp/TeMp | grep gateway | grep conf | tail -n1 | awk -F"(" '{ print $2 }' | cut -d":" -f1 | xargs /usr/bin/sudo -i -u $installusercomm  grep ServerName | grep -v ^\# | awk -F"ServerName" '{ print $2 }' )
 echo COMM_VT-GATEWAY_SERVERALIAS \; $(cat /tmp/TeMp | grep gateway | grep conf | tail -n1 | awk -F"(" '{ print $2 }' | cut -d":" -f1 | xargs /usr/bin/sudo -i -u $installusercomm  grep ServerAlias | grep -v ^\# | awk -F"ServerAlias" '{ print $2 }' )
else
...
if [ -f "$httpdCOMM" ];
then
 grep -v ^\# $(cat /tmp/TeMp | grep gateway | grep conf | awk -F"(" '{ print $2 }' | cut -d":" -f1 | uniq) | /usr/bin/sudo -i -u $installusercomm  grep vt-uas | grep DocumentRoot > /tmp/TeMp3
 TEST=$(cat /tmp/TeMp3 | awk '{ print $2 }')

if [ -f "$TEST/index.php" ];
then
  vtUasVer=$(/usr/bin/sudo -i -u $installusercomm grep Version $TEST/index.php | awk -F":" '{ print $2 }')
  echo "COMM_VT-UAS_VERSION ;" $vtUasVer
else
  echo "COMM_VT-UAS_VERSION ; NA"
fi

echo COMM_VT-UAS_SERVERNAME \; $(cat /tmp/TeMp | grep gateway | grep conf | tail -n1 | awk -F"(" '{ print $2 }' | cut -d":" -f1 | xargs /usr/bin/sudo -i -u $installusercomm  grep ServerName | grep -v ^\# | awk -F"ServerName" '{ print $2 }' )
echo COMM_VT-UAS_SERVERALIAS \; $(cat /tmp/TeMp | grep gateway | grep conf | tail -n1 | awk -F"(" '{ print $2 }' | cut -d":" -f1 | xargs /usr/bin/sudo -i -u $installusercomm  grep ServerAlias | grep -v ^\# | awk -F"ServerAlias" '{ print $2 }' )
...
if [[ $1_SISTEMAOPERATIVO =~ "Linux 8.*" ]]; then
  echo "$1_IPADDRESS; hostname -I"
  else
  echo "$1_IPADDRESS; $IP"
fi
...
# then 
#  echo "CORE_VTMIDDLEWARE_VERSION;" $(sudo -i -u vt724 $installpath/vt-net/bin/deviceHandler -v  )
# else
...
if [ -d $installpath ];
 then
  VTNETBIN="$installpath/vt-net/bin/kernel"

if [ -f "$VTNETBIN" ];
 then 
  echo "CORE_VTNET_VERSION;" $(sudo -i -u vt724 LD_LIBRARY_PATH=$installpath/vt-net/lib:$installpath/vt-net-notif/lib:$installpath/share/lib:$installpath/share/instantclient_11/lib/:$installpath/share/instantclient_12/lib/:$installpath/share/instantclient_18/lib/:$installpath/share/instantclient_19/lib/ $installpath/vt-net/bin/kernel -v  )
 else
...
if [ -d $installpath ];
 then
  VTMIDDLEWAREBIN="$installpath/vt-net/bin/deviceHandler"

if [ -f "$VTMIDDLEWAREBIN" ];
 then 
  echo "CORE_VTMIDDLEWARE_VERSION;" $(sudo -i -u vt724 LD_LIBRARY_PATH=/$installpath/vt-net/lib:/$installpathnotif/vt-net-notif/lib:/veritran/share/lib:$installpathauth/share/instantclient_11/lib/:$installpathauth/share/instantclient_12/lib/:$installpathauth/share/instantclient_18/lib/:$installpathauth/share/instantclient_19/lib/ $installpath/vt-net/bin/deviceHandler -v  )
 else
	echo "CORE_VTMIDDLEWARE_VERSION;NA"
fi 
fi
...
if [ -d $installpath ];
 then
  TNSDBS=$(sudo -i -u vt724 grep -v \# $installpath/share/network/admin/tnsnames.ora | awk -F"[ =]" '/DESCRIPTION/ { print X }{ X=$1 }')
if [ -n "$TNSDBS" ];
 then 
  ParseaTNS $1 $TNSDBS
 else
...
if [ -f "$VTNETASBIN" ];
 then 
  echo "AUTH_VTNETAS_VERSION;" $(sudo -i -u vt724 LD_LIBRARY_PATH=/$installpath/vt-net/lib:/$installpathnotif/vt-net-notif/lib:/veritran/share/lib:$installpathauth/share/instantclient_11/lib/:$installpathauth/share/instantclient_12/lib/:$installpathauth/share/instantclient_18/lib/:$installpathauth/share/instantclient_19/lib/ $installpathauth/vt-net-as/bin/kernel -v  | awk '{ print $2 }' )
 else
...
if [ -f "$VTASBIN" ];
 then 
  echo "AUTH_VTAS_VERSION;" $(sudo -i -u vt724 LD_LIBRARY_PATH=/$installpath/vt-net/lib:/$installpathnotif/vt-net-notif/lib:/veritran/share/lib:$installpathauth/share/instantclient_11/lib/:$installpathauth/share/instantclient_12/lib/:$installpathauth/share/instantclient_18/lib/:$installpathauth/share/instantclient_19/lib/ $installpathauth/vt-net-as/bin/tkAuthServer -v  | awk '{ print $2 }' )
 else
...
if [ -r $installpathnotif ];
 then
  ...
  if [ -f "$VTNETNOTIFBIN" ];
   then 
     NOTIFVERSION=$(sudo -i -u vt724 LD_LIBRARY_PATH=/$installpath/vt-net/lib:$installpathnotif/vt-net-notif/lib:/veritran/share/lib:$installpathnotif/share/instantclient_11/lib/:$installpathnotif/share/instantclient_12/lib/:$installpathnotif/share/instantclient_18/lib/:$installpathnotif/share/instantclient_19/lib/ $installpathnotif/vt-net-notif/bin/kernel -v )
     notifregex="1.13.5.*"
...
if [ -f "$VTNOTIFBIN" ];
 then 
  echo "NOTIF_VTNOTIF_VERSION;" $(sudo -i -u vt724 LD_LIBRARY_PATH=/$installpath/vt-net/lib:/$installpathnotif/vt-net-notif/lib:/veritran/share/lib:$installpathauth/share/instantclient_11/lib/:$installpathauth/share/instantclient_12/lib/:$installpathauth/share/instantclient_18/lib/:$installpathauth/share/instantclient_19/lib/ $installpathnotif/vt-net-notif/bin/vtNotifServer -v  | awk '{ print $2 }' )
