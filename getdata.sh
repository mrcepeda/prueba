...
declare -A processes
processes['CORE']='kernel.*N[A-Z][A-Z]1'
processes['AUTH']='kernel.*N[A-Z][A-Z]2'
processes['COMM']='(appD.*N[A-Z][A-Z]3)|(lighttpd-comm)'
processes['NOTIF']='kernel.*N[A-Z][A-Z]4'
processes['OMNI']='workspace-api'
processes['STUDIO']='tomcat-juli.jar'

for proc in "${!processes[@]}"; do
  if pgrep -f "${processes[$proc]}" >/dev/null; then
    "${proc}GetData" "$proc"
  fi
done
